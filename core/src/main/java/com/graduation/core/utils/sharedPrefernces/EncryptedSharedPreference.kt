...
class EncryptedSharedPreference @Inject constructor(@ApplicationContext context: Context) {
    ...
    private fun saveUserData(value: LocalUser?) {
        val json = Gson().toJson(value)
        saveData(USER_DATA, json)
    }

    private fun loadUserData(): LocalUser {
        val value = loadData<String>(USER_DATA)
        val innerUser = Gson().fromJson(value, LocalUser::class.java)
        return innerUser ?: LocalUser()
    }
